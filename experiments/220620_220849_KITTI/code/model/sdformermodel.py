...
class SDFORMERModel(nn.Module):
    def __init__(self, args):
        ...
        self.output = nn.Conv2d(int(self.dim * 3 ** 1), self.out_channels, kernel_size=3, stride=1, padding=1, bias=self.bias)

        self.conv1_rgb = conv_bn_relu(3, 9, kernel=3, stride=1, padding=1, bn=False)

        self.conv1_dep = conv_bn_relu(1, 3, kernel=3, stride=1, padding=1, bn=False)
...
if __name__ == '__main__':
    ...
    print(model)
    rgb = torch.randn((1, 3, 240, 1216)).to(device)
    dep = torch.randn((1, 1, 240, 1216)).to(device)
