    ...
    def save(self, epoch, idx, sample, output):
        with torch.no_grad():
            if self.args.save_result_only:
                ...
                np.save(('{}/01_rgb'.format(self.path_output)), np.squeeze(rgb))
                np.save(('{}/02_dep'.format(self.path_output)), np.squeeze(dep))
                np.save(('{}/03_pred_final'.format(self.path_output)), np.squeeze(pred))
                np.save(('{}/05_gt'.format(self.path_output)), np.squeeze(gt))
                
                plt.imsave('{}/03_pred_fina_jet.png'.format(self.path_output), np.squeeze(pred), cmap='jet')
                plt.imsave(('{}/05_gt_jet.png'.format(self.path_output)), np.squeeze(gt), cmap='jet')
                ...
                path_save_pred_gray = '{}/04_pred_final_gray.png'.format(self.path_output)
                path_save_gt = '{}/05_gt.png'.format(self.path_output)
